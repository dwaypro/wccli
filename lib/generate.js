var fs = require('fs'),
    ...
    const webComponentDocumentation = fs.readFileSync(path.join(path.dirname(require.main.filename), '/assets/schematics/readme/readme.md'), 'utf-8');
...
function determineSchematic(options, name){
    if(options.v){
        return [
            ...
            {file: webComponentDocumentation, ext: '.readme.md'}
        ]    
    }else if(options.vc){
